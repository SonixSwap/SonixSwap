...
import { useApproveXOS } from 'hooks/useApprove'
import useI18n from 'hooks/useI18n'
import {useStakeXOS} from 'hooks/useStake'
import {useUnStakeXOS} from 'hooks/useUnstake'
import { useHarvestXOS } from 'hooks/useHarvest'
...
const PoolCard: React.FC<HarvestProps> = ({ pool }) => {
  ...
  const xosprice = userData?.xosprice || 0
  const { onApprove } = useApproveXOS(stakingTokenContract)
  const { onStake } = useStakeXOS(salsaId,xosprice)
  const { onUnstake } = useUnStakeXOS(salsaId)
  const { onReward } = useHarvestXOS()
  ...
  const totalStaking = new BigNumber(userData?.totalStaking || 0)
  const unstakeBalance = new BigNumber(userData?.unstakeBalance || 0)
  ...
  const [onPresentWithdraw] = useModal(
    <WithdrawModal max={unstakeBalance} onConfirm={onUnstake} tokenName={stakingTokenName} />,
  )
    ...
    <Card isActive={isCardActive} isFinished={isFinished && salsaId !== 0}>
      ...
      <div style={{ padding: '24px' }}>
        <CardTitle isFinished={isFinished && salsaId !== 0}>
          ...
          {TranslateString(330, `Earn ${earnToken}`)}
        </CardTitle>
        ...
        <StyledCardActions>
          ...
          {account &&
            (needsApproval&&!requestedApproval? (
              <div style={{ flex: 1 }}>
        ...
        <StyledDetails>
          ...
          <Balance fontSize="14px" isDisabled={isFinished} value={getBalanceNumber(stakedBalance, 8)} />
        </StyledDetails>
      </div>
      <CardFooter
