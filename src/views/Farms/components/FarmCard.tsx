...
import { Button, Flex, Image, Link } from '@kiwifinancebsc/uikit'
import { communityFarms } from 'config/constants'
import { Farm } from 'state/types'
import { usePriceBnbBusd, usePriceKiwiBusd } from 'state/hooks'
  ...
  return (
    <FCard>
      ...
      <Label>
        ...
        <span className="right">{farm.lpSymbol && farm.lpSymbol.toUpperCase().replace('KIWIFINANCE', '')}</span>
      </Label>
      <Label>
        <span>{TranslateString(318, 'Earn')}</span>
        <span className="right">{farm.dual ? farm.dual.earnLabel : 'KIWI'}</span>
      </Label>
      {!removed && (
        <Label>
